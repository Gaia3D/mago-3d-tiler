...
public class OldLasConverter {
    public List<GaiaPointCloudOld> load(String path) {
        return convert(new File(path));
    }

    public List<GaiaPointCloudOld> load(File file) {
        return convert(file);
    }

    public List<GaiaPointCloudOld> load(Path path) {
    ...
    public void loadToTemp(GaiaPointCloudHeader pointCloudHeader, File file) {
        ...
        try {
            ...
            log.warn("[Pre] Doesn't detect Coordinate Reference System from LAS header. {}", file.getName());
        ...
        for (LASPoint point : pointIterable) {
            ...
            if (tempFile == null) {
                log.debug("[ERROR] Failed to find temp file.");
            } else {
    ...
    private List<GaiaPointCloudOld> convert(File file) {
        List<GaiaPointCloudOld> pointClouds = new ArrayList<>();
        GaiaPointCloudOld pointCloud = new GaiaPointCloudOld();
