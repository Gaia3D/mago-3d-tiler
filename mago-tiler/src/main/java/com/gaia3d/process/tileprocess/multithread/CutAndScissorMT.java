...
public class CutAndScissorMT {
    ...
    public CutAndScissorResult apply(
        ...
        CompletionService<CutTaskResult>
                completionService =
        ...
        /*
         * Aquí ya han terminado todos los workers.
         * El cálculo de anchors es completamente secuencial.
         */
        boundaryAnchorsByLod =
    ...
    buildGlobalBoundaryAnchorsByLod(
        ...
        if (planeCutResultsByLod == null
                || planeCutResultsByLod.isEmpty()
                || tileInfosByLod == null
                || rootNodeBBoxLC == null) {
            return result;
        }

        for (Map.Entry<Integer, PlaneCutResult> entry
                : planeCutResultsByLod.entrySet()) {
            ...
            List<TileInfo> lodTileInfos =
                    tileInfosByLod.get(lod);

            if (lodTileInfos == null
                    || lodTileInfos.isEmpty()) {

                log.warn(
                        "Cannot build global anchors. "
                                + "No TileInfo found for LOD {}",
                        lod
                );
            ...
            /*
             * Primero comprobamos únicamente los puntos
             * generados por intersecciones interiores.
             */
            builder.addPoints(
            ...
            log.info(
                    "Global boundary anchors built. "
                            + "LOD={}, cuttingPoints={}, anchors={}",
                    lod,
                    planeCutResult.getCuttingPoints().size(),
                    globalAnchors.size()
            );
        }
    ...
    private CutTaskResult processSingleTile(
        ...
        log.info(
                "[Tile][PhotoRealistic] Cut RectangleCake: {}",
...
//    private Map<Integer, GlobalAnchorPoints>
//    calculateGlobalAnchorPointsByLod(
//            Map<Integer, List<LocalPlaneCutResult>>
//                    localResultsByLod
//    ) {
//        Map<Integer, GlobalAnchorPoints> result =
//                new HashMap<>();
//
//        for (Map.Entry<Integer, List<LocalPlaneCutResult>> entry
//                : localResultsByLod.entrySet()) {
//
//            int lod =
//                    entry.getKey();
//
//            List<LocalPlaneCutResult> sourceResults =
//                    entry.getValue();
//
//            GlobalAnchorPoints globalAnchorPoints =
//                    new GlobalAnchorPoints(lod);
//
//            for (LocalPlaneCutResult sourceResult
//                    : sourceResults) {
//
//                PlaneCutResult planeCutResult =
//                        sourceResult.planeCutResult();
//
//                Matrix4d localToWorld =
//                        sourceResult.localToWorldMatrix();
//
//                for (Vector3d cuttingPoint
//                        : planeCutResult.getCuttingPoints()) {
//
//                    Vector3d pointWC =
//                            new Vector3d(cuttingPoint);
//
...
//                     * Aquí:
//                     *
//                     * 1. transformar WC a root-local, si ese es
//                     *    el sistema del CellGrid3D;
//                     *
//                     * 2. obtener cellKey;
//                     *
//                     * 3. añadir el candidato a GlobalAnchorPoints.
//                     */
//                    globalAnchorPoints.addWorldCandidate(
//                            pointWC
//                    );
//                }
//            }
//
//            /*
//             * Aquí puedes hacer deduplicación, promedios,
//             * selección determinista y congelado.
//             */
//            globalAnchorPoints.finish();
//
//            result.put(
//                    lod,
//                    globalAnchorPoints
