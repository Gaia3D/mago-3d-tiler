...
public class GeometryOnlyReMesherByOctreeV2 {
    ...
    private int minFrontierTriangleUseCountToReMesh = 3;

    public GeometryOnlyReMesherByOctreeV2() {
    }

    private int[] countVertexTriangleUseCount(
            List<GaiaFace> faces,
            int vertexCount
    ) {
        ...
        for (GaiaFace face : faces) {
            ...
            Set<Integer> usedInThisFace = new HashSet<>();
...
//        areaFoldRatio = 3.43007347681656
    ...
    public void reMeshPrimitive(
        ...
        int[] vertexTriangleUseCount = countVertexTriangleUseCount(
                faces,
                vertices.size()
        );
        ...
        GaiaOctreeFaces octreeFaces = new GaiaOctreeFaces(
                null,
                cubeBoundingBox
        );
        ...
        List<GaiaOctree<GeometryContent>> octreesWithContent =
                octreeFaces.extractOctreesWithContents();

        Set<GaiaVertex> innerVerticesToReMesh = new HashSet<>();
        Set<GaiaVertex> frontierVerticesToReMesh = new HashSet<>();
        ...
        for (GaiaOctree<GeometryContent> octree : octreesWithContent) {
            ...
            innerVerticesToReMesh.clear();
            frontierVerticesToReMesh.clear();
            ...
            for (int i = 0; i < facesCount; i++) {
                ...
                for (int j = 0; j < indices.length; j++) {
                    ...
                    if (frontierVertices[vertexIndex]) {
                        if (vertexTriangleUseCount[vertexIndex]
                                >= minFrontierTriangleUseCountToReMesh) {
                            frontierVerticesToReMesh.add(vertex);
                        }

                        continue;
                    }

                    innerVerticesToReMesh.add(vertex);
                }
            }
            ...
            if (innerVerticesToReMesh.size() < 2 &&
                    frontierVerticesToReMesh.size() < 2) {
                continue;
            }
            ...
            if (!reMeshAnyWay) {
                ...
                if (barInfo.isBar) {
                    log.debug(
                            "Slim object detected by PCA. Skip reMesh it. slenderness = "
                                    + barInfo.slenderness
                    );
                    continue;
                }
                ...
                if (isBuildingWallProtected(shapeInfo, stats)) {
                    log.debug(
                            "Building wall detected by AABB and statistics. Skip reMesh it"
                    );
                    continue;
                }
            }
            ...
            if (innerVerticesToReMesh.size() >= 2) {
                ReMesh(innerVerticesToReMesh.stream().toList());
            }

            if (frontierVerticesToReMesh.size() >= 2) {
                ReMesh(frontierVerticesToReMesh.stream().toList());
            }
        }
    }
