...
public final class FrontierClub {
     ...
     * WEST  = minX
     */
    public static FrontierClub classify(
            Vector3d position,
            GaiaBoundingBox nodeBox,
            double tolerance
    ) {
        ...
        EnumSet<Side> detectedSides =
                EnumSet.noneOf(Side.class);

        double distanceToNorth =
                Math.abs(
                        position.y
                                - nodeBox.getMaxY()
                );

        double distanceToSouth =
                Math.abs(
                        position.y
                                - nodeBox.getMinY()
                );

        double distanceToEast =
                Math.abs(
                        position.x
                                - nodeBox.getMaxX()
                );

        double distanceToWest =
                Math.abs(
                        position.x
                                - nodeBox.getMinX()
                );
    ...
    public Side getSingleSide() {
        if (!isSingle()) {
            throw new IllegalStateException(
                    "FrontierClub does not contain exactly one side: "
                            + sides
            );
        }
