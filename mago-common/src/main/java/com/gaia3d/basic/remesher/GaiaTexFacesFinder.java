...
public class GaiaTexFacesFinder {
    ...
    public static List<Island> findIslands(GaiaSurface surface) {
        ...
        long[] edgeRecords = new long[totalTriangleCount * 3];
        int edgeRecordCount = 0;
        ...
        for (int faceIndex = 0; faceIndex < facesCount; faceIndex++) {
            ...
            for (int i = 0; i + 2 < indices.length; i += 3) {
                ...
                if (i0 != i1) {
                    edgeRecords[edgeRecordCount++] = makeEdgeRecord(i0, i1, faceIndex);
                }

                if (i1 != i2) {
                    edgeRecords[edgeRecordCount++] = makeEdgeRecord(i1, i2, faceIndex);
                }

                if (i2 != i0) {
                    edgeRecords[edgeRecordCount++] = makeEdgeRecord(i2, i0, faceIndex);
                }
            }
        }

        if (edgeRecordCount == 0) {
            return buildIslandsFromUnionFind(faces, unionFind);
        }

        Arrays.sort(edgeRecords, 0, edgeRecordCount);

        int start = 0;

        while (start < edgeRecordCount) {
            long edgeKey = getEdgeKeyFromRecord(edgeRecords[start]);

            int end = start + 1;

            while (end < edgeRecordCount &&
                    getEdgeKeyFromRecord(edgeRecords[end]) == edgeKey) {
                end++;
            }

            int firstFaceIndex = getFaceIndexFromRecord(edgeRecords[start]);

            for (int i = start + 1; i < end; i++) {
                int otherFaceIndex = getFaceIndexFromRecord(edgeRecords[i]);
                unionFind.union(firstFaceIndex, otherFaceIndex);
            }
