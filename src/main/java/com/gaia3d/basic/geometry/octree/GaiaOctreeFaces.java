...
public class GaiaOctreeFaces {
    private GaiaOctreeFaces parent = null;
    ...
    private GaiaOctreeFaces[] children = null;
    // children indices
    // down                         up
    // +---------+---------+        +---------+---------+
    // |         |         |        |         |         |
    ...
    public GaiaOctreeFaces(GaiaOctreeFaces parent) {
        if (parent != null) {
            this.parent = parent;
    ...
    public void createChildren() {
        children = new GaiaOctreeFaces[8];
        for (int i = 0; i < 8; i++) {
            children[i] = new GaiaOctreeFaces(this);
            children[i].idx = i;
        }
...
//        distributeContentsByBoundingBox(distributionUnique);
    ...
    public GaiaOctreeFaces getRoot() {
        GaiaOctreeFaces root = null;
    ...
    public GaiaOctreeFaces getOctreeByCoordinate(GaiaOctreeCoordinate coord) {
        if (coord.getDepth() < 0) {
            return null;
        }

        GaiaOctreeFaces resultOctree = null;
    ...
    public GaiaOctreeFaces getOctreeByFullPath(List<GaiaOctreeCoordinate> fullPath) {
        if (fullPath == null || fullPath.isEmpty()) {
            return null;
        }

        GaiaOctreeFaces root = this.getRoot();
        GaiaOctreeFaces current = root;
    ...
    public boolean[] hasNeighbor() {
        ...
        GaiaOctreeFaces parent = this.parent;
        if (parent == null) {
            return result;
        }
